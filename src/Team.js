...
const Team = () => {
  ...
  return (
    <>
          ...
          listMode ? (
                            ...
                            <div>
                              <div className="flex items-center justify-between">
                                <p className="text-sm text-gray-900">
                                  Limit Spent:{" "}
                                  {member.percentageSpent.toFixed(1)}%
                                </p>
                                <p className="text-xs text-gray-500">
                                  ${member.totalSpent} / ${member.limit}{" "}
                                </p>
                              </div>
                              {/* {member.totalSpent === 0 ? (
                              ...
                              <div className="w-full h-2 mt-3 rounded-full bg-lime-100">
                                <div
                                  ...
                                  style={{
                                    width:
                                      member.percentageSpent >= 100
                                        ? "100%"
                                        : `${member.percentageSpent}%`,
                                  }}
                                  // style={{ width: '50%' }}
            ...
            <ul className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
              {team.map((person) => (
                <li
                  key={person.email}
                  className="flex flex-col col-span-1 text-center bg-white border divide-y divide-gray-200 rounded-lg cursor-pointer hover:bg-slate-50"
                >
                  <div className="flex flex-col flex-1 p-8">
                    {person.imageUrl ? (
                      <img
                        className="flex-shrink-0 w-32 h-32 mx-auto rounded-full mx-atuo"
                        src={person.imageUrl}
                        alt=""
                      />
                    ) : (
                  ...
                  <div className="py-6 mx-4">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-xs text-slate-400">
                        ${person.totalSpent} / ${person.limit}{" "}
                      </p>
                    </div>
                    <div className="w-full h-4 mt-3 rounded-md bg-lime-100">
                      <div
                        style={{
                          width:
                            person.percentageSpent >= 100
                              ? "100%"
                              : `${person.percentageSpent}%`,
                        }}
      ...
      {/* Slideout */}
      <NewTeamMember
        openNewMember={openNewMember}
        setOpenNewMember={setOpenNewMember}
      />
    </>
  );
};
