...
const LedgerContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (team) {
      ...
      const MembersSpent = team.map((member) => {
        ...
        let membersTransactions = transactions.filter(
          (transaction) => transaction.purchased_by === member.name
        );
        ...
        let totalSpent = transactionsAmount.reduce(
  ...
  const removeTransaction = (id) => {
    ...
    setTransactions(
      transactions.filter((transaction) => transaction.transaction_id !== id)
    );
  };
